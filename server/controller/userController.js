...
let loginUser = async(req,res)=>{
    ...
    if(!findUser){
        if(role=="admin") return res.send({message:"you are not admin"})
        let userData = {}
        ...
        let token = JWT.sign({ userId: userId, role:role }, "ashish-r-jwt", {expiresIn: 86400});
        console.log(token);
       ...
       let newPass = data.password
        ...
        let token = JWT.sign({ userId: userId, role:findUser.role },"ashish-r-jwt", {expiresIn: 86400});
